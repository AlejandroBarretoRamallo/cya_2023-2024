...
int main(int argc, char *argv[]) {
  ...
  typedef std::vector<std::vector<Estado>> conjuntos_estados;
  conjuntos_estados conjuntos_estados_1;
  std::vector<Estado> estadoq;
  estadoq.push_back(NFA.GetVector()[estado_arranque]);
      ...
      for (int j = 0; j < conjuntos_estados_1.size(); ++j) {
        if (estadoq.size() == conjuntos_estados_1[j].size()) {
          for (int k = 0; k < estadoq.size(); ++k) {
            if (estadoq[k].GetNtransiciones() == conjuntos_estados_1[j][k].GetNtransiciones()) {
              if(estadoq[k].IsAccepted() == conjuntos_estados_1[j][k].IsAccepted()) {
                if(estadoq[k].GetMap() == conjuntos_estados_1[indice][k].GetMap()) {
                  pertenece = 1;
                }
              } 
            }
