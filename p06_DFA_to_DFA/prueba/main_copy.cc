...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0 ; i < conjuntos_estados_1.size(); ++i) {
    ...
    for (int j = 0; j < conjuntos_estados_1[i].size(); ++j) {
      std::multimap<char, int> map = NFA.GetVector()[conjuntos_estados_1[i][j]].GetMap();
      auto simbolo_ = map.find('&');
      if (simbolo_ != map.end()) {
        auto rango_ = map.equal_range('&');
        std::set<int> set_;
        for (auto iterador = rango_.first; iterador != rango_.second; ++ iterador) {
          set_.insert(iterador -> second);
        }
        ntransiciones += set_.size();
      }
    }
    DFA << ntransiciones << " ";
    for (const char elemento : alfabeto_FA.GetSet()) {
